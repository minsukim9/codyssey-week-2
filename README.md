...
프로젝트 완료 후 원격 저장소를 별도의 로컬 디렉터리에 clone했습니다.

복제한 저장소에서 README를 수정하고 원격 저장소에 push한 뒤, 기존 작업 디렉터리에서 pull하여 변경사항이 정상적으로 반영되는 것을 확인했습니다.

## 추가 기능

### 문제 무작위 출제

퀴즈를 시작할 때 문제 순서를 무작위로 구성합니다.

`random.sample()`을 사용하기 때문에 기존 퀴즈 목록의 저장 순서는 변경되지 않으며, 같은 게임에서 동일한 문제가 중복으로 출제되지 않습니다.

### 문제 수 선택

퀴즈를 시작하기 전에 사용자가 풀 문제 수를 직접 선택할 수 있습니다.

선택 가능한 범위는 `1`부터 현재 등록된 전체 퀴즈 수까지이며, 선택한 개수만큼 문제가 무작위로 출제됩니다.

### 힌트 기능

정답을 입력하는 단계에서 `h`를 입력하면 해당 문제의 힌트를 확인할 수 있습니다.

- 한 문제당 힌트는 한 번만 사용할 수 있습니다.
- 힌트 1회당 최종 점수에서 5점이 차감됩니다.
- 최종 점수는 0점 미만으로 내려가지 않습니다.
- 새로운 퀴즈를 추가할 때 힌트도 함께 등록합니다.

점수는 다음과 같이 계산합니다.

```text
기본 점수 = 정답 개수 / 전체 문제 개수 × 100
최종 점수 = 기본 점수 - (힌트 사용 횟수 × 5)
```

### 퀴즈 삭제

등록된 퀴즈 목록에서 원하는 문제를 선택하여 삭제할 수 있습니다.

삭제하기 전에 선택한 문제를 다시 보여주고 사용자에게 최종 확인을 받습니다. 삭제된 결과는 즉시 `state.json`에 저장되어 프로그램을 다시 실행해도 유지됩니다.

### 점수 기록 히스토리

최고 점수뿐만 아니라 모든 퀴즈 플레이 결과를 저장합니다.
...
점수 기록은 최신 기록부터 확인할 수 있으며, 프로그램을 종료한 뒤 다시 실행해도 유지됩니다.
...
퀴즈 게임 프로그램의 실행 진입점입니다.

`QuizGame` 객체를 생성하고 게임을 실행합니다.

### `quiz_game/quiz.py`

하나의 퀴즈를 표현하는 `Quiz` 클래스를 정의합니다.
...
퀴즈 게임의 전체 실행 흐름을 관리하는 `QuizGame` 클래스를 정의합니다.
...
### `quiz_game/game_record.py`

한 번의 퀴즈 플레이 결과를 표현하는 `GameRecord` 클래스를 정의합니다.
...
- 힌트 사용 횟수 관리
- 게임 기록을 딕셔너리로 변환
- JSON 데이터를 게임 기록 객체로 변환
- 저장된 기록 데이터의 유효성 검사

### `quiz_game/input_handler.py`

사용자 입력과 입력값 검증을 담당합니다.
...
저장 파일이 없거나 손상되었을 때 사용할 기본 퀴즈 데이터를 제공합니다.

---

## 데이터 파일

퀴즈 데이터, 최고 점수와 전체 점수 기록은 프로젝트 루트의 `state.json` 파일에 UTF-8 인코딩으로 저장됩니다.
...
```json
{
    ...
    "score_history": [
        {
            "played_at": "2026-08-03T20:16:00+09:00",
            "score": 95,
            "correct_count": 5,
            "total_count": 5,
            "hint_count": 1
...
| `hint` | 문제의 힌트 |
| `best_result` | 현재 최고 점수 정보 |
| `score_history` | 전체 퀴즈 플레이 기록 |
| `played_at` | 퀴즈를 플레이한 날짜와 시간 |
| `score` | 힌트 감점이 반영된 최종 점수 |
| `correct_count` | 맞힌 문제 수 |
| `total_count` | 출제된 전체 문제 수 |
| `hint_count` | 힌트 사용 횟수 |

기존 데이터에 `hint`, `hint_count`, `score_history` 필드가 없어도 기본값을 적용하여 정상적으로 불러올 수 있습니다.
